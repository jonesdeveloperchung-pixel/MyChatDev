...
## 11. Sandboxed Development Environment

**Current State:** The development process involves a linear handoff from the Programmer to a Static Analysis node and then to a Tester. This is inefficient and the feedback loops are longer than necessary. The Programmer agent is not empowered to validate its own work.

**Suggestion:** Redesign the development phase into a single, powerful, self-contained node called the **"Sandboxed Development Environment"**. This creates a micro-environment where a more advanced, tool-using Programmer agent can work iteratively to produce code that is guaranteed to be technically valid.

**Implementation Details:**

1.  **The Sandbox Environment:**
    *   For each development task, a temporary, isolated directory is created on the filesystem.
    *   This directory is pre-populated with the necessary context, such as the design documents and the unit tests generated by the Tester agent.

2.  **The Tool-Using Programmer Agent:**
    *   The Programmer agent will be upgraded to a "tool-using" agent. Instead of just outputting code, it will be given access to a set of tools it can use to perform actions within the sandbox.
    *   **Available Tools:**
        *   `write_file(filename, content)`
        *   `read_file(filename)`
        *   `run_shell_command(command)`
        *   `list_directory(path)`
        *   `submit_deliverable()` (A special tool to signal completion)

3.  **The Inner Loop - Self-Correction in the Sandbox:**
    *   The Programmer's high-level goal is redefined: **"Make the tests pass."**
    *   The agent works autonomously inside the sandbox node, following a cycle of "code, compile, test, debug":
        1.  It uses `write_file` to create source files and a `Makefile`.
        2.  It uses `run_shell_command` to execute `make` or `gcc`.
        3.  It inspects the command's output. If there are compiler errors, it reads its own code (`read_file`), debugs the error, writes the fix (`write_file`), and retries the compilation.
        4.  Once the code compiles, it uses `run_shell_command` to execute the unit tests.
        5.  It inspects the test results. If tests fail, it debugs the logic, applies fixes, and repeats the cycle.
    *   This inner loop continues until the agent confirms that all tests pass.

4.  **Exiting the Sandbox:**
    *   Once the agent is satisfied with its work, it calls the `submit_deliverable()` tool. This action successfully concludes the "Sandboxed Development" node.
    *   The final, validated code from the sandbox is then passed to the main workflow for the next phase (Holistic Review).

5.  **Configuration:**
    *   A new `enable_sandbox: bool` flag should be added to the `SystemConfig` to allow users to enable or disable this feature.

**Benefits:**
*   **Highest-Quality Feedback:** The agent receives direct, undeniable feedback from real-world tools (the compiler and test runner).
*   **Increased Autonomy:** The Programmer agent is empowered to solve problems on its own within its environment, reducing the need for longer, more expensive feedback loops involving other agents.
*   **Guaranteed Technical Validity:** Code that exits the sandbox is guaranteed to compile and pass its unit tests, significantly increasing the quality of the input for the final review phase.
*   **Simplified Workflow Graph:** The main workflow graph becomes cleaner, as the complex logic of the development cycle is abstracted away into a single, powerful node.
