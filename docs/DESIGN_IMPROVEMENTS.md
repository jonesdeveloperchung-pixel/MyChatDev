...
1.  **Configurable Distiller Role:**
    *   Introduce a new `distiller` role in the `AVAILABLE_LLMS` in `src/config/settings.py`. This allows the user to select a specific model for the summarization task.
    *   Example:
        ```python
        "distiller": LLMConfig(
            name="Distiller",
...
3.  **Progressive Distillation Algorithm:**
    *   The `compress_content` function in `src/models/llm_manager.py` will be updated to implement this algorithm.
...
2.  **Full-Context Evaluation:**
    ...
    *   **Solution:** Remove the truncation in `_format_state` within `src/workflow/quality_gate.py`. The quality gate should receive the full, intelligently compressed project state (using the "Progressive Distillation" method).
...
**Current State:** Some internal system agents, such as the `QualityGate`, have their underlying LLM model hardcoded directly in the source code (e.g., `gemma3:12b` in `src/workflow/quality_gate.py`).

**Suggestion:** Eliminate all hardcoded model IDs for system agents. All agents, including the `QualityGate`, `Summarizer`/`Distiller`, and any future internal agents, should be configurable through the `AVAILABLE_LLMS` dictionary in `src/config/settings.py`.
...
2.  **Enhanced CLI Arguments:**
    ...
    *   The command `python cli.py` now displays a simple help message instead of running the full workflow.
    *   To run the workflow, the user must now use the `python cli.py --demo` command.
