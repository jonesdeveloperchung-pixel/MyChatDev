    ...
    try:
        ...
        if final_state.quality_evaluations:
            ...
            if len(final_state.quality_evaluations) > 1:
                first_quality = final_state.quality_evaluations[0].get(
                    "quality_score", 0
                )
                logger.info(
                    f"Quality improvement: {first_quality:.2f} → {final_quality:.2f}"
                )
            ...
            if final_quality >= 0.8:
                ...
                logger.warning(
                    f"⚠️ Quality threshold not met: {final_quality:.2f} < 0.8"
                )
                return False
        else:
