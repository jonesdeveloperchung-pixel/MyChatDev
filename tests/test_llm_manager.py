...
class TestLLMManager:
    ...
    def llm_config(self):
        ...
        """
        return LLMConfig(
            name="Test Model", model_id="test:model", role="tester", temperature=0.5
        )
    ...
    def system_config(self):
        ...
        """
        return SystemConfig(ollama_host="http://localhost:11434", max_iterations=3)
    ...
    async def test_check_model_availability_success(self, llm_manager):
        ...
        mock_response = {"models": [{"name": "test:model"}, {"name": "other:model"}]}

        # Patch the client's list method to return the mock response.
        with patch.object(
            llm_manager.client, "list", new_callable=AsyncMock
        ) as mock_list:
    ...
    async def test_generate_response_success(self, llm_manager, llm_config):
        ...
        with patch.object(
            llm_manager, "check_model_availability", new_callable=AsyncMock
        ) as mock_check:
            ...
            with patch.object(
                llm_manager.client, "chat", new_callable=AsyncMock
            ) as mock_chat_method:
    ...
    async def test_batch_generate_success(self, llm_manager, llm_config):
        ...
        with patch.object(
            llm_manager, "generate_response", new_callable=AsyncMock
        ) as mock_generate:
            mock_generate.side_effect = ["Response 1", "Response 2"]

            results = await llm_manager.batch_generate(requests)

            assert results == {"req1": "Response 1", "req2": "Response 2"}
...
def test_llm_config_validation():
    ...
    # Valid configuration
    config = LLMConfig(name="Test", model_id="test:model", role="tester")

    assert config.name == "Test"
    assert config.temperature == 0.7  # default value
    assert config.max_tokens == 131072  # default value
