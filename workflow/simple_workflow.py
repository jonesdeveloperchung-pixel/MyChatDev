...
class SimpleCooperativeLLM:
    ...
    async def run(self, user_input: str) -> WorkflowState:
        ...
        try:
            ...
            while (
                not state.should_halt
                and state.iteration_count < self.config.max_iterations
            ):
                # Run workflow steps
                ...
                if state.should_halt:
                    if state.iteration_count < 2 and len(state.quality_evaluations) > 0:
                        last_quality = state.quality_evaluations[-1].get(
                            "quality_score", 0
                        )
                        if last_quality < self.config.quality_threshold:
                            self.logger.info(
                                f"🔄 Quality {last_quality:.2f} < {self.config.quality_threshold}, forcing iteration {state.iteration_count + 1}"
                            )
